...
def import_referrals():
    """Import historical referrals from CSV"""
    imported_count = 0
    skipped_count = 0
    
    with open('historical_referrals.csv', 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            try:
                ...
                referral_data = {
                    'id': referral_id,  # Use 'id' as primary key
                    'name': row['referral_name'].strip(),
                    'email': f"{row['referral_name'].lower().replace(' ', '.')}@example.com",  # Placeholder email
                ...
                if imported_count % 10 == 0:
                    print(f"Imported {imported_count} referrals...")
...
if __name__ == "__main__":
    print("Starting historical referral data import...")
    import_referrals()